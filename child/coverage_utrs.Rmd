...
```{r table_xx_utr_read_support, results='asis'}
table_xx_utr_read_support <- cbind(c("5'UTR", "3′ UTR"), 
            c(sum(utr5_stats_combined$num_reads, na.rm=TRUE),
              sum(utr3_stats_combined$num_reads, na.rm=TRUE)) / 1E6)
colnames(table_xx_utr_read_support) <- c("Feature", "Number of reads (millions)")

xkable(table_xx_utr_read_support, caption='**Table X**: Number of reads supporting the primary UTR boundaries (millions)')
```

```{r figure_xx, fig.show='hold', out.width='450px', fig.cap="**Figure X**: Distribution of number of SL- and Poly(A)-containing reads.", eval=FALSE}
# NOT USED (Dec 2016)
...
```{r table_x_coverage_summary_table, results='asis'}
...
# summary data frame
table_xx_percent_genes_with_features_assigned <- cbind(N=nvals, SL=sl_pcts, PolyA=polya_pcts)

xkable(table_xx_percent_genes_with_features_assigned, caption='**Table X**: Percent genes containing N or more reads mapping to the specified feature.')
```
