...
```{r results='asis'}
cat(sprintf('- Median distance from SL acceptor site: %d\n', median(polypyr_combined$sl_dist)))
cat(sprintf('- Median distance from Poly(A) site: %d\n', median(polypyr_combined$polya_dist)))
```
...
```{r polpyrimidine_tract_logo_global, fig.width=7680/192}
# perform an MSA
...
```{r, results='asis', cache=TRUE, autodep=TRUE, fig.width=10, fig.height=5.625, dpi=192}
