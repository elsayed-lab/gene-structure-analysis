...
for (stage in c('combined', stages)) {
	...
	median_5utr <- median(utr5_stats_filtered[[stage]]$length, na.rm=TRUE)
	median_cds  <- median(as.numeric(genes$size))
	median_3utr <- median(utr3_stats_filtered[[stage]]$length, na.rm=TRUE)
	...
	summary_dat <- gene_structure_summary(utr5_stats_filtered[[stage]]$length,
										as.numeric(genes$size),
										utr3_stats_filtered[[stage]]$length)
