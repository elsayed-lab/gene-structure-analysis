...
table_01 <- num_reads %>% 
    ...
    summarize("Human (millions)"=sum(num_reads_host, na.rm=TRUE) / 1E6,
              "Parasite (millions)"=sum(num_reads_parasite) / 1E6,
              "SL-containing parasite reads (%)"=mean(pct_reads_sl),
              "Poly(A)-containing parasite reads (%)"=mean(pct_reads_polya)) %>%
    rename("Parasite Stage"=stage)

xkable(table_01, caption='**Table 1**: Summary of mapping statistics.')
```
