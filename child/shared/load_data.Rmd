...
noncoding_ids <- genes$Name[grepl(CONFIG$id_filter_string, genes$Name)]

ncrna_mask <- rep(FALSE, length(genes))

if (!is.null(CONFIG$id_filter_string)) {
    ncrna_mask <- ncrna_mask | grepl(CONFIG$id_filter_string, genes$Name)
}
