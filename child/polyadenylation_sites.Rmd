...
table_X9 <- merge(primary_site_props, alt_site_props, by='polya_site') %>%
    arrange(desc(primary))
colnames(table_X9) <- c('Polyadenylation sequence', 'Primary site', 'Minor site')
kable(table_X9, caption='**Table X9** Polyadenylation site dinucleotide sequence frequencies for primary and alternative sites.')
```
...
```{r figure_S4, fig.width=3840/192, fig.height=720/192, dpi=192, fig.cap="**Figure S4** Sequence logo for region surrounding alternative poly(A) sites."}
...
table_X10 <- polya_dists %>% 
    group_by(orientation) %>%
    summarize(number=n(), proportion=n() / nrow(polya_dists))
kable(table_X10, caption='**Table X10** Distribution of upstream and downstream alternative polyadenylation sites')
...
```{r figure_S5_primary_alt_polya_site_dist_stage_specific, results='asis', fig.cap='**Figure S5** Distribution of distances between primary and alternative polyadenylation sites in transcripts from different developmental stages of T. cruzi'}
plts <- list()
i = 1
