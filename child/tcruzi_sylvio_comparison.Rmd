...
dat <- dat %>% 
    ...
    filter(length_diff > 20 & min_num_reads > 5)

# plot 5' UTR lengths
ggplot(dat, aes(x=ystrain_length, y=sylvio_length, color=min_num_reads)) +
    geom_point() +
    scale_colour_gradient2(limits=c(NA, quantile(dat$min_num_reads, 0.95)),
...
table_X13 <- merge(table_X13, gene_info, by='gene')

kable(table_X13, caption="**Table X13** Genes with significantly differing 5' UTR lengths between T. cruzi strains")
```
...
# Print table of interesting hits
table_X14 <- dat %>% filter(abs(ystrain_length - sylvio_length) > 100 &
                            min_num_reads > 10)

# add gene description and print list of interesting hits
table_X14 <- merge(table_X14, gene_info, by='gene')

kable(table_X14, caption="**Table X14** Genes with significantly differing 3' UTR lengths between T. cruzi strains")
```
