...
for x in $SCRATCH/utr_analysis/tcruzi_ystrain/tcruzi-infecting-hsapiens-*/input/HPGL0*; do
    ...
    # SL
    infiles=($SCRATCH/utr_analysis/tcruzi_ystrain/*/*spliced_leader/minlength3_mindiff-2/${sample_id}*/results/matched_reads_*.csv)
    num_reads=$(wc -l $infiles | tail -1 | egrep -o "[0-9]+")
	printf "%s,%d\n" $sample_id $num_reads >> $sl_outfile

    # Poly(A)
    infiles=($SCRATCH/utr_analysis/tcruzi_ystrain/*/poly*/minlength3_mindiff-2/${sample_id}*/results/matched_reads_*.csv)
    num_reads=$(wc -l $infiles | tail -1 | egrep -o "[0-9]+")
