---
title: Trypanosome Gene Structure Analysis (L. major 1.0)
...
```{r include=FALSE}
library(knitr)
knit('settings/lmajor_hsapiens.Rmd', quiet=TRUE, output=tempfile())
```
...
for (table_name in ls()[grepl('^table', ls())]) {
    ...
    write.csv(get(table_name), file=file.path(table_dir, filename))
}
